...
export class RegistroAdmin implements OnInit {
  ...
  public actualizar(){
    ...
    if(Object.keys(this.errors).length > 0){
     ...
     console.log("ADMIN QUE ENVÍAS:", this.admin);
    ...
    this.administradoresService.actualizarAdmin(this.admin).subscribe({
      next: (response) => {
        ...
        console.log(response);
        //this.cargarAdmin(); // Volvemos a cargar los datos del administrador para asegurarnos de que se muestren los datos actualizados en la vista
